...
const StoreContextProvider = (props) => {
  ...
  const fetchInfo = async () => {
    try {
      const response = await fetch("https://demowebsite-9lag.onrender.com/products");
      if (response.ok) {
...
const getDiscountedPrice = (product) => {
  ...
  const addToCart = async (product) => {
    ...
    if (cart) {
      ...
      const res = await fetch("https://demowebsite-9lag.onrender.com/cart", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const Wishlist = async (product) => {
    ...
    try {
      ...
      if (data.length) {
        ...
        const res = await fetch("https://demowebsite-9lag.onrender.com/wishlist", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
